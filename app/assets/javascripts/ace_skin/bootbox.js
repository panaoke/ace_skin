...
window.bootbox = window.bootbox || (function init($, undefined) {
    ...
    each(buttons, function(key, button, index) {
      ...
      if (!button.className) {
        if (total <= 2 && index === total-1) {
          ...
          button.className = "btn-default btn btn-sm";
        } else {
  ...
  exports.confirm = function() {
    ...
    options.title = _t('CONFIRM_INFO');
    options.className = 'confirm_info';
  ...
  exports.dialog = function(options) {
    ...
    each(buttons, function(key, button) {
      ...
      var btn = $("<button data-bb-handler='" + key + "' type='button' class='btn " + button.className + "'>" + button.label + "</button>");
      each(button.html || {} , function(k, v) {
          btn.attr(k, v)
      });

      buttonStr += btn[0].outerHTML;
    ...
    */
    ...
    dialog.on("click", ".modal-footer button", function(e) {
      ...
      if($(this).data("bootbox-confirm") != undefined) {
          window.bootbox.confirm($(this).data("bootbox-confirm"), function(result) {
              if(result) { processCallback(e, dialog, callbacks[callbackKey]) };
          });
      }else{
