...
//Helper functions
        ...
        createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
            ...
            mh.className = "ui-jqdialog-titlebar ui-widget-header ui-corner-all";
...
//		} else {
//			mw.dir = "ltr";
//			$(".ui-jqdialog-title",mh).css("fquery_conditionsloat","left");
        ...
        createEl : function(eltype,options,vl,autowidth, ajaxso) {
            ...
            switch (eltype)
            {
                ...
                case "select" :
                    ...
                    elem.setAttribute("data-toggle","choose");
                    elem.setAttribute("data-width","110px");
                    var msl, ovm = [];
    ...
    $.fn.jqFilter = function( arg ) {
        ...
        var p = $.extend(true,{
            ...
            direction : "ltr"
                ...
                if ($("div.up",this).size() == 0) {
                 var upDIV = $("<div class='up'></div>");
                 $(this).append(upDIV);
                };
                  if ($("div.down",this).size() == 0) {
                  var downDIV = $("<div class='down'></div>");
                  $(this).append(downDIV)
                };
        ...
        return this.each( function() {
            ...
            this.reDraw = function() {
                var that = this;
                $("div.query_conditions",this).remove();
                if($(this).find('.seachBtnPanel').size() == 0) {
                    var pannel = this.createSearchBtn(p.filter, null);
                    $(upDIV).append(pannel);
                }
                ...
                $("a.close_pannel",this).remove();
                var close_pannel = $("<a  href='#' class='close_pannel'><i class='icon-external-link-sign'></i>" +
                i18n.view('jg_grid','closeSearchPanel') +"</a>");

                var open_pannel = $("<a  href='#' class='open_pannel'><i class='icon-plus-sign-alt'></i>"+
                i18n.view('jg_grid','openSearchPanel') +"</a>");
                $("a.open_pannel",this).remove();
                open_pannel.bind('click', function() {
                   // $("#" + p.id).trigger('showSearchPanel', true);
                   $(upDIV).show();
                   $(downDIV).hide();
                });
               
                $(upDIV).append(close_pannel);
                close_pannel.bind('click', function() {
                    var fid = "fbox_"+ p.id;
                    // $(that).find('#'+fid+'_reset').trigger('click');
                    that.resetFilter();
                    // $("#" + p.id).trigger('showSearchPanel', false);
                    $(upDIV).hide();
                    $(downDIV).show();
                });
                $(open_pannel).extPlugins();
                $(downDIV).append(open_pannel);
                $("div.query_conditions",this).extPlugins();
            };
            this.createSearchBtn = function (group) {
                var that = this, i;
                var div = $("<div class='seachBtnPanel'></div>");
                var fid = "fbox_"+ p.id;

                var groupOpSelect = $("<select class='opsel'></select>");

                var resetBtn  = $("<a id="+fid+"_search class='btn btn-info btn-xs'>" +
                    "<span class='ui-icon ui-icon-search'></span><i class='icon-search icon-on-right bigger-110'>"+"查找"+"</i></a>");
                var queryBtn  = $("<a id="+fid+"_reset class='btn btn-xs btn-yellow'>" +
                    "<span class='ui-icon ui-icon-arrowreturnthick-1-w'></span><i class='icon-refresh bigger-110'></i>"+"重置"+"</a>");
                ...
                if(this.p.ruleButtons === true) {
                    ...
                    var inputAddRule = $("<a class='btn btn-xs btn-primary'><i class='icon-plus bigger-110'></i>" + "添加"+ "</a>"), cm;
                    inputAddRule.bind('click',function() {
                        group = p.filter
                        //if(!group) { group = {};}
                        if (group.rules === undefined) { group.rules = [] }
                ...
                div.append(groupOpSelect).append(inputAddRule).append(queryBtn).append(resetBtn);
                return div;
            };
            ...
            this.createTableForGroup = function(group, parentgroup) {
                ...
                var list = $("<div class='query_conditions row'></div>"),
                ...
                // 	conditionDiv.append(inputDeleteGroup);
                ...
                // 		list.append(trHolderForSubgroup);
            ...
            this.createTableRowForRule = function(rule, group ) {
                ...
                var that=this, $t = getGrid(),
                //document.createElement("tr"),

                // first column used for padding
                //tdFirstHolderForRule = document.createElement("td"),
                    i, op, trpar, cm, str="", selected;
                var conditionDiv = $("<div class='query_condition col-sm-4'></div>");
                ...
                var ruleFieldSelect = $("<select data-toggle='choose' data-width='100px' data-disable_search='true'></select>"), ina, aoprs = [];
                conditionDiv.append(ruleFieldSelect);
                ruleFieldSelect.bind('change',function() {
                    ...
                    setTimeout(function(){ //IE, Opera, Chrome
                        // by aoke
                        rule.dataType = cm.searchoptions.dataType || 'string';
                        rule.data = $(elm).val();
                    }, 0);
...
//            ruleFieldSelect.trigger('change');
                ...
                var ruleOperatorSelect = $("<select class='selectopts' data-toggle='choose' data-width='80px' " +
                "data-disable_search='true'></select>");
                conditionDiv.append(ruleOperatorSelect);
                ...
                // conditionDiv.append(ruleDataTd);

                // textbox for: data
                // is created previously
                //ruleDataInput.setAttribute("type", "text");
                var ruleDataDiv = $("<span class='data'></span>")
                ruleDataDiv.append(ruleDataInput);
                conditionDiv.append(ruleDataDiv);
                $.jgrid.bindEv.call($t, ruleDataInput, cm.searchoptions);
                $(ruleDataInput)
                    .addClass("input-elm").data('toggle', 'choose')
                    .bind('change', function() {
                ...
                // 有一些插件初始化时已经赋值,所以需要触发change事件来获取input的value. by aoke
                $(ruleDataInput).trigger('change');


                // create action container
                // var ruleDeleteTd = $("<td></td>");
                // conditionDiv.append(ruleDeleteTd);

                // create button for: delete rule
                if(this.p.ruleButtons === true) {
                    var ruleDeleteInput = $("<a href='#' class='icon-remove'><a>");
                    conditionDiv.append(ruleDeleteInput);
            ...
            this.resetFilter = function (showFlag) {
                this.p.filter = $.extend(true,{},this.p.initFilter);
                this.reDraw(showFlag != undefined);
                this.onchange();
            };
...
//					$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{gbox:"#gbox_"+$.jgrid.jqID(fid),jqm:p.jqModal, modal:p.modal, overlay: p.overlay, toTop: p.toTop});
                ...
                if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
                    ...
                    var fil = $("<div class='jpGrid-'><div id='"+fid+"' class='searchFilter'></div></div>").insertBefore("#gview_"+$.jgrid.jqID($t.p.id)),
                        align = "left", butleft ="";
                    ...
                    if( (!defaultFilters && colnm) || p.multipleSearch === false  ) {
                        ...
                        if(cmi >=0 && columns[cmi].searchoptions && columns[cmi].searchoptions.sopt) {
                            ...
                            dataType = columns[cmi].searchoptions.dataType || 'string';
                        } else if(p.sopt && p.sopt.length) {
                            cmop = p.sopt[0];
                        }
                        defaultFilters = {groupOp: "AND", rules: [{field: colnm, op: cmop, data: "", dataType: dataType}]};
                    }
                    found = false;
            ...
            return this.each(function() {
                ...
                if ($("#"+alertIDs.themodal)[0] === undefined) {
                    ...
                    $.jgrid.createModal(alertIDs,
                            "<div>"+o.alerttext+"</div><span><span id='jqg_alrt'></span></span>",
                        {
                ...
                for(i = 0; i<clone; i++) {
                    ...
                    if (o.search) {
                        ...
                        $(tbd,navtbl).trigger('click');
                        this._searchFilter.showOrHide();
                        // $(tbd,navtbl).trigger('click');
                        // $(tbd,navtbl).click();
